...
const index_1 = require("./helper-classes/index");
...
const { log } = apify_1.default.utils;
...
apify_1.default.main(async () => {
    var _a;
    const input = /** @type {typedefs.Input} */ (await apify_1.default.getValue('INPUT'));
    ...
    walker, 
    // Scraping options
    includeHistogram = false, includeOpeningHours = false, includePeopleAlsoSearch = false, maxReviews = 0, maxImages = 1, exportPlaceUrls = false, additionalInfo = false, maxCrawledPlaces = 99999999, maxCrawledPlacesPerSearch = maxCrawledPlaces, maxAutomaticZoomOut, reviewsTranslation = 'originalAndTranslated', 
    ...
    // Personal data
    scrapeReviewerName = true, scrapeReviewerId = true, scrapeReviewerUrl = true, scrapeReviewId = true, scrapeReviewUrl = true, scrapeResponseFromOwnerText = true, } = input;
    if (debug) {
        log.setLevel(log.LEVELS.DEBUG);
    }
    const { stats, errorSnapshotter, placesCache, maxCrawledPlacesTracker, exportUrlsDeduper, } = await (0, index_1.initializeHelperClasses)({ cachePlaces, cacheKey, useCachedPlaces, maxCrawledPlaces, maxCrawledPlacesPerSearch, exportPlaceUrls });
    // Requests that are used in the queue, we persist them to skip this step after migration
    const startRequests = /** @type {Apify.RequestOptions[]} */ (await apify_1.default.getValue('START-REQUESTS')) || [];
    ...
    if (startRequests.length === 0) {
        ...
        if (Array.isArray(startUrls) && startUrls.length > 0) {
            ...
            const rlist = await apify_1.default.openRequestList('STARTURLS', updatedStartUrls);
            for (;;) {
            ...
            for (const placeId of placesCache.placesInPolygon(geolocation, maxCrawledPlaces, searchStringsArray)) {
                const searchString = searchStringsArray.filter(x => { var _a; return (_a = placesCache.place(placeId)) === null || _a === void 0 ? void 0 : _a.keywords.includes(x); })[0];
                startRequests.push({
        ...
        for (const request of startRequests) {
            if (((_a = request.userData) === null || _a === void 0 ? void 0 : _a.label) === 'detail') {
        ...
        await apify_1.default.setValue('START-REQUESTS', startRequests);
        const apifyPlatformKVLink = 'link: https://api.apify.com/v2/key-value-stores/'
            + `${apify_1.default.getEnv().defaultKeyValueStoreId}/records/START-REQUESTS?disableRedirect=true`;
        const localLink = 'local disk: apify_storage/key_value_stores/default/START-REQUESTS.json';
        // @ts-ignore Missing type in SDK
        const link = apify_1.default.getEnv().isAtHome ? apifyPlatformKVLink : localLink;
    ...
    const proxyConfiguration = await apify_1.default.createProxyConfiguration(proxyConfig);
